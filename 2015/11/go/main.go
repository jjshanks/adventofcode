...
import (
	...
	"jjshanks.net/adventofcode/v2/2015/11/go/challenge"
)
...
func main() {
	...
	x, t := runAndTime(func() string { return challenge.PartOne(challengeInput) })
	outputString += color.BlueString(x)
	if t > 15 || forceTime {
	...
	x, t = runAndTime(func() string { return challenge.PartTwo(challengeInput) })
	outputString += color.BlueString(x)
	if t > 15 || forceTime {
		outputString += fmt.Sprintf(" in %s seconds", color.BlueString(fmt.Sprintf("%.8f", t)))
	}
...
func runTests(infoStruct info) {
	...
	rt := func(tcs []tc, f func(string) string, n string) {
		for i, tc := range tcs {
			...
			result, t := runAndTime(func() string { return f(tc.Input) })

			if result == tc.Expected {
				outputString += color.GreenString("pass")
			} else {
				outputString += fmt.Sprintf("%s (got %s, expected %s)", color.RedString("fail"), color.BlueString(result), color.BlueString(tc.Expected))
			}
...
func runAndTime(f func() string) (string, float64) {
	st := time.Now()
	x := f()
	et := time.Now()
